...
if (name === 'salesPerson') {
  ...
  useEffect(() => {
    ...
    const handleSubmit = async (e) => {
      e.preventDefault();
      try {
        if (!formData.cusCode || !formData.invoiceDate || tableData.length === 0) {
          throw new Error('Please fill in all required fields and add at least one product.');
        }

        const invalidProducts = tableData.filter(row => !row[3]); // Assuming product ID is in the 4th column (index 3)
        ...
        const invoiceData = {
          ...
          invoiceQty: tableData.reduce((total, row) => total + parseInt(row[6], 10), 0), // Assuming quantity is in the 7th column (index 6)
          totalAmount: tableData.reduce((total, row) => total + parseFloat(row[5] || 0) * parseInt(row[6] || 0), 0), // Price * Quantity
          productId: productId,
          cusId: cusId,
          stockId: stockId
        };
