...
CREATE TABLE IF NOT EXISTS `posSystem`.`supplier` (
  ...
  `supplierPaid` FLOAT NOT NULL,
  `supplierBalance` FLOAT NOT NULL,
  `supplierStatus` VARCHAR(45) NOT NULL,
  `supplierPaymentDate` DATETIME NOT NULL,
...
CREATE TABLE IF NOT EXISTS `posSystem`.`invoice` (
  ...
  INDEX `fk_invoice_transaction1_idx` (`transaction_transactionId` ASC),
  INDEX `fk_invoice_user1_idx` (`user_userId` ASC),
  ...
  CONSTRAINT `fk_invoice_transaction1`
    ...
    REFERENCES `posSystem`.`transaction` (`transactionId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `posSystem`.`grn` (
  ...
  `grnCashAmount` INT NOT NULL,
  `grnChequeAmount` INT NOT NULL,
  `grnChequeNumber` VARCHAR(45) NOT NULL,
  `grnDueAmount` INT NOT NULL,
  `grnVat` FLOAT NOT NULL,
  `grnDiscount` FLOAT NOT NULL,
  `grnTotal` FLOAT NOT NULL,
  `grnGrandTotal` FLOAT NOT NULL,
  `products_productId` INT NOT NULL,
