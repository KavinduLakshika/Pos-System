...
CREATE TABLE IF NOT EXISTS `posSystem`.`transaction` (
  ...
  `rentalInvoice_rentalInvoiceId` INT NOT NULL,
  PRIMARY KEY (`transactionId`),
  INDEX `fk_transaction_invoice1_idx` (`invoice_invoiceId` ASC),
  INDEX `fk_transaction_supplier1_idx` (`supplier_supplierId` ASC),
  INDEX `fk_transaction_rentalInvoice1_idx` (`rentalInvoice_rentalInvoiceId` ASC),
  CONSTRAINT `fk_transaction_invoice1`
    FOREIGN KEY (`invoice_invoiceId`)
    REFERENCES `posSystem`.`invoice` (`invoiceId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_transaction_supplier1`
    FOREIGN KEY (`supplier_supplierId`)
    REFERENCES `posSystem`.`supplier` (`supplierId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_transaction_rentalInvoice1`
    FOREIGN KEY (`rentalInvoice_rentalInvoiceId`)
    REFERENCES `posSystem`.`rentalInvoice` (`rentalInvoiceId`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
